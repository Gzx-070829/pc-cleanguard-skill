...
- **v0.2**：受控 cleanup planning + external tool adapter foundations。
  ...
  - **v0.2 PR14**：显式路径 junk candidate scanner、保护目录阻断、cleanup preview 和 dry-run CLI。
  - **v0.2 PR15**：默认 dry-run、显式确认、allow-root 和强制审计约束下的 L1 temp/cache/log 文件清理。
  - **当前 PR — v0.2 PR16**：合成垃圾 demo、完整 preview/execute/audit/report 试用闭环、清理摘要与 Markdown 报告。
